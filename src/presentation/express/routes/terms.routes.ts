...
router
  .route('/')
  .post(authenticateRequest, routesHandler(createTermsOfServiceController))
  .get(routesHandler(findManyTermsController));

router
  .route('/bulk')
  .delete(authenticateRequest, routesHandler(deleteManyTermsController));

router
  .route('/:id')
  .put(authenticateRequest, routesHandler(updateTermsController))
  .delete(authenticateRequest, routesHandler(deleteTermsController))
  .get(routesHandler(findTermsController));
