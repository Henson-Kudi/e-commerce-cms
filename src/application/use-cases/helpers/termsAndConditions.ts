...
import { IFindTermsAndConditionsDTO } from '../../../domain/dtos/termsAndConditions';
import slugify from '../../../utils/slugify';

export function setupFindManyTermsAndConditonsQuery(
  data: IFindTermsAndConditionsDTO
) {
  const query: Prisma.TermsOfServiceWhereInput = {
  ...
  if (data.title)
    query.slug = Array.isArray(data.title)
      ? { in: data.title.map((item) => slugify(item)) }
      : slugify(data.title);
