import { ICreateTermsAndConditionsDTO, IFindTermsAndConditionsDTO, IFindTermsAndConditionsOptions, IUpdateTermsAndConditionsDTO } from "../../domain/dtos/termsAndConditions"
...
export default class TermsOfService {
    ...
    getManyTermsOfService(filter: IFindTermsAndConditionsDTO, options: IFindTermsAndConditionsOptions) {
    ...
    createTermsOfService(data: ICreateTermsAndConditionsDTO) {
        return new CreateTermsOfServiceUseCase(this.repo, { messageBroker: messageBroker }).execute(data)
    }

    updateTermsOfService(id: string, data: IUpdateTermsAndConditionsDTO) {
        return new UpddateTermsOfService(this.repo, { messageBroker: messageBroker }).execute(id, data)
    }

    deleteTermsOfService(id: string, options?: { soft?: boolean }) {
        return options?.soft ? new SoftDeleteTermsOfService(this.repo, { messageBoker: messageBroker }).execute(id) : new DeleteTermsOfService(this.repo, { messageBoker: messageBroker }).execute(id)
    }

    deleteManyTermsOfService(ids: string[], options?: { soft?: boolean }) {
        return options?.soft ? new SoftDeleteManyTermsOfService(this.repo, { messageBoker: messageBroker }).execute(ids) : new DeleteManyTermsOfService(this.repo, { messageBoker: messageBroker }).execute(ids)
    }
}
