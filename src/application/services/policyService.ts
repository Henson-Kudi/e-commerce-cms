import { ICreatePolicyDTO, IFindPolicyDTO, IFindPrivacyOptions, IUpdatePolicyDTO } from "../../domain/dtos/privacyPolicy"
...
export default class PolicyService {
    ...
    createPolicy(data: ICreatePolicyDTO) {
        return new CreatePolicyUseCase(this.repo, { messageBroker: messageBroker }).execute(data)
    }

    updatePolicy(id: string, data: IUpdatePolicyDTO) {
        return new UpddatePolicy(this.repo, { messageBroker: messageBroker }).execute(id, data)
    }

    deletePolicy(id: string, options?: { soft?: string }) {
        return options?.soft ? new SoftDeletePolicy(this.repo, { messageBoker: messageBroker }).execute(id) : new DeletePolicy(this.repo, { messageBoker: messageBroker }).execute(id)
    }

    deletePolicies(ids: string[], options?: { soft?: string }) {
        return options?.soft ? new SoftDeletePolicies(this.repo, { messageBoker: messageBroker }).execute(ids) : new DeletePolicies(this.repo, { messageBoker: messageBroker }).execute(ids)
    }
}
