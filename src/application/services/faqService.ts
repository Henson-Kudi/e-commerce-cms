...
export default class FaqService {
  ...
  createFaq(data: ICreateFaqDTO) {
    return new CreateFaqUseCase(this.repo, {
      messageBroker: messageBroker,
    }).execute(data);
  }

  updateFaq(id: string, data: IUpdateFaqDTO) {
    return new UpddateFaq(this.repo, { messageBroker: messageBroker }).execute(
      id,
      data
    );
  }
  ...
  deleteFaq(id: string, options?: { soft?: boolean }) {
    return options?.soft
      ? new SoftDeleteFaq(this.repo, { messageBoker: messageBroker }).execute(
          id
        )
      : new DeleteFaq(this.repo, { messageBoker: messageBroker }).execute(id);
  }

  deleteFaqs(ids: string[], options?: { soft?: boolean }) {
    return options?.soft
      ? new SoftDeleteFaqs(this.repo, { messageBoker: messageBroker }).execute(
          ids
        )
      : new DeleteFaqs(this.repo, { messageBoker: messageBroker }).execute(ids);
  }
}
