...
export default class BlogPostService {
  ...
  createPost(data: ICreateBlogDTO) {
    return new CreateBlogUseCase(this.blogsRepo, {
      messageBroker: messageBroker,
    }).execute(data);
  }

  updatePost(query: IFindUniqueBlogDTO, data: IUpdateBlogDTO) {
    return new UpddateBlog(this.blogsRepo, {
      messageBroker: messageBroker,
    }).execute(query, data);
  }

  updateManyPosts(query: IFindBlogDTO, data: IUpdateBlogDTO) {
    return new UpddateBlogs(this.blogsRepo, {
      messageBroker: messageBroker,
    }).execute(query, data);
  }
  ...
  deletePost(query: IFindUniqueBlogDTO, options?: { soft?: boolean }) {
    return options?.soft
      ? new SoftDeleteBlog(this.blogsRepo, {
          messageBoker: messageBroker,
        }).execute(query)
      : new DeleteBlog(this.blogsRepo, { messageBoker: messageBroker }).execute(
          query
        );
  }
  ...
  deletePosts(query: IFindBlogDTO, options?: { soft?: boolean }) {
    return options?.soft
      ? new SoftDeleteBlogs(this.blogsRepo, {
          messageBoker: messageBroker,
        }).execute(query)
      : new DeleteBlogs(this.blogsRepo, {
          messageBoker: messageBroker,
        }).execute(query);
  }
}
