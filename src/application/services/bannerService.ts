...
export default class BannersService {
  ...
  createBanner(data: ICreateBannerDTO) {
    return new CreateBannerUseCase(this.repo, {
      messageBroker: messageBroker,
    }).execute(data);
  }

  updateBanner(id: string, data: IUpdateBannerDTO) {
    return new UpddateBanner(this.repo, {
      messageBroker: messageBroker,
    }).execute(id, data);
  }
  ...
  deleteBanner(id: string, options?: { soft?: boolean }) {
    return options?.soft
      ? new SoftDeleteBanner(this.repo, {
          messageBoker: messageBroker,
        }).execute(id)
      : new DeleteBanner(this.repo, { messageBoker: messageBroker }).execute(
          id
        );
  }
  ...
  deleteBanners(ids: string[], options?: { soft?: boolean }) {
    return options?.soft
      ? new SoftDeleteBanners(this.repo, {
          messageBoker: messageBroker,
        }).execute(ids)
      : new DeleteBanners(this.repo, { messageBoker: messageBroker }).execute(
          ids
        );
  }
}
